    ...
    public class UGOPool
    {
        ...
        private void Cull()
        {
            if (UGOPoolUtill.IsDebug)
            {
                ...
                {
                    if (usedItemList[i].TryGetTarget(out GameObject gObj) && gObj == null)
                    {
        ...
        public void ReleaseItem(GameObject item)
        {
            if (item == null)
            {
                Debug.LogError("GameObjectPool::ReleaseItem->Item is Null");
                return;
            }
            ...
            if (UGOPoolUtill.IsDebug)
            {
                ...
                for (int i = usedItemList.Count - 1; i >= 0; i--)
                {
                    if (usedItemList[i].TryGetTarget(out GameObject target) && target!=null)
                    {
                        if (target != item)
                        {
                            continue;
