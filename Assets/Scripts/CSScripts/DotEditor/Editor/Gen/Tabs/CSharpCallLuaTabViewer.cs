...
namespace DotEditor.Lua
{
    internal class CSharpCallLuaTabViewer : GenTabViewer
    {
        ...
        protected internal override void OnGUI(Rect rect)
        {
            ...
            {
                genericTypeRL = new ReorderableList(genConfig.CSharpCallLuaGenericTypes, typeof(string), true, true, true, true);
                ...
                genericTypeRL.drawElementCallback = (r, index, isActive, isFocuse) =>
                {
                    ...
                    genConfig.CSharpCallLuaGenericTypes[index] = EditorGUI.TextField(typeNameRect, genConfig.CSharpCallLuaGenericTypes[index]);

                    Rect btnRect = new Rect(r.x + r.width - 60, r.y, 60, r.height);
                    if (GUI.Button(btnRect, "Check"))
                    {
                        string typeName = genConfig.CSharpCallLuaGenericTypes[index];
                        bool isValid = true;
            ...
            GUILayout.BeginArea(rect);
            {
                scrollPos = EditorGUILayout.BeginScrollView(scrollPos, EditorStyles.helpBox);
                {
                    ...
                    foreach (var typeData in assemblyTypes)
                    {
                        ...
                        bool isSelected = genConfig.CSharpCallLuaTypes.IndexOf(fullName) >= 0;
                        bool tempIsSelected = EditorGUILayout.ToggleLeft(typeData.Type.FullName, isSelected);
                        if (tempIsSelected != isSelected)
                        {
                            if (tempIsSelected)
                            {
                                genConfig.CSharpCallLuaTypes.Add(fullName);
                            }
                            else
                            {
                                genConfig.CSharpCallLuaTypes.Remove(fullName);
                            }
                        }
                    }
                }
