...
namespace DotEditor.Lua
{
    internal class LuaCallCSharpTabViewer : GenTabViewer
    {
        ...
        protected internal override void OnGUI(Rect rect)
        {
            ...
            {
                genericTypeRL = new ReorderableList(genConfig.LuaCallCSharpGenericTypes, typeof(string), true, true, true, true);
                ...
                genericTypeRL.drawElementCallback = (r, index, isActive, isFocuse) =>
                {
                    ...
                    genConfig.LuaCallCSharpGenericTypes[index] = EditorGUI.TextField(typeNameRect, genConfig.LuaCallCSharpGenericTypes[index]);
                   
                    Rect btnRect = new Rect(r.x + r.width - 60, r.y, 60, r.height);
                    if(GUI.Button(btnRect,"Check"))
                    {
                        string typeName = genConfig.LuaCallCSharpGenericTypes[index];
                        bool isValid = true;
            ...
            GUILayout.BeginArea(rect);
            {
                scrollPos = EditorGUILayout.BeginScrollView(scrollPos, EditorStyles.helpBox);
                {
                    ...
                    foreach(var typeData in assemblyTypes)
                    {
                        ...
                        bool isSelected = genConfig.LuaCallCSharpTypes.IndexOf(fullName) >= 0;
                        bool tempIsSelected = EditorGUILayout.ToggleLeft(typeData.Type.FullName, isSelected);
                        if(tempIsSelected!=isSelected)
                        {
                            if(tempIsSelected)
                            {
                                genConfig.LuaCallCSharpTypes.Add(fullName);
                            }
                            else
                            {
                                genConfig.LuaCallCSharpTypes.Remove(fullName);
                            }
                        }
                    }
                }
